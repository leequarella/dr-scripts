class Disarm
  ...
  def disarm
    ...
    waitrt?
    @disarmed = disarmed? check
    disarm unless @disarmed || @sprung
  end

  def disarmed? check
    if(check =~ /lock springs out and stabs you painfully in the finger/ ||
    ...
    check =~ /You feel like you've done a good job of blocking up the pinholes, until you peer closely to examine /)
     ...
     check =~ /not yet fully disarmed/ ||
     check =~ /for a while but are unable to make any progress/)
      @disarmed = false

    elsif(check =~ /not yet fully disarmed/ ||
    check =~ /proves too difficult to manipulate/)
      @disarmed   = true
      @more_traps = true
